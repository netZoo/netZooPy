...
######
    def __init__(self, expression_file, methylation_file, ppi_file, motif_file='tests/milipeed/MotifPrior_CGmap.txt', computing='cpu',precision='double',start=1, end=None,save_dir='milipeed_output/', save_fmt='txt'):
        # =====================================================================
        # Data loading
        # =====================================================================
        ...
        for iii in self.indexes:
            ...
            with Timer('Calculating coexpression network ...'):
                ...
                #         self.lionish_network=np.float32(self.lionish_network)
                    ...
                    # subset_correlation_network = self._normalize_network(np.corrcoef(self.expression_data.values[:, idx]))
                    subset_correlation_network = ((self.num_subj-1) * (correlation_matrix) - subj_exp.T * subj_exp) /(self.num_subj-2)
