...
######
    def __init__(self, expression_file, methylation_file, motif_file, ppi_file, map_file='tests/milipeed/MotifPrior_CGmap.txt', computing='cpu',precision='double',save_mem=True, full_motif=True, start=1, end=None,save_dir='milipeed_output/', save_fmt='txt'):
        # =====================================================================
        # Data loading
        # =====================================================================
        ...
        if methylation_file is not None and expression_file is not None:
            with Timer('Finding common subjects ...'):
                ...
                self.b_subj=tmp_betadata[tmp_betadata.columns[tmp_betadata.columns.isin(tmp_expdata.columns)]]
                self.e_subj=tmp_expdata[tmp_expdata.columns[tmp_expdata.columns.isin(tmp_betadata.columns)]]
        ...
        # self.indexes    = range(len(b_subj.columns))#range(self.num_subj)[start-1:end] 
...
###
    def milipeed_loop(self):
        ...
        with Timer('Creating motif ...'):
            ...
            self.mdata=pd.read_csv(methylation_file,sep=' ',index_col=0,header=0,usecols=[0,list(b_subj).index(iii)+2])##this matches held-out expression sample in lioness
            ...
            #         if self.precision=='single':
            #             self.ppi_matrix=np.float32(self.ppi_matrix)
            self.processData(self,modeProcess='intersection',expression_file, motif_file=self.mdata, ppi_file,remove_missing=True,keep_expression_matrix=False)

            for iii in list(b_subj): #self.indexes:
                ...
                with Timer('Calculating coexpression network ...'):
                    ...
                    #     subset_correlation_network = ((self.num_subj-1) * (correlation_matrix) - subj_exp.T * subj_exp) /(self.num_subj-2)
                    ...
                    if self.expression_data is not None & self.correlation_matrix is not None:# and self.computing=='cpu':
                        ...
                        #faster and no gpu
                        subset_correlation_network = ((self.num_subj-1) * (self.correlation_matrix) - subj_exp.T * subj_exp) /(self.num_subj-2)

                        self.lionish_network = self.num_subj * (self.correlation_matrix - subset_correlation_network) + subset_correlation_network
                        if self.precision=='single':
                            self.lionish_network=np.float32(self.lionish_network)
