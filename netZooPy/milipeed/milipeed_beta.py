    ...
    def __init__(self, expression_file, motif_file, ppi_file, save_memory = False, save_tmp=True):
        ...
        if methylation is not None:
            with Timer('Loading methylation data ...'):
                self.methyl_data = pd.read_csv(methylation, sep='\t', header=None)
                self.unique_cgs = sorted(set(self.methyl_data[0]))
                self.num_cgs = len(self.unique_cgs)
                print('Unique CGs:', self.num_cgs)
        else:
