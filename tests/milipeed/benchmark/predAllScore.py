...
def main(indir='data/MotifPipeline/sthlm_motif5f',outdir='data/MotifPipeline/test/'):
    ...
    for jac,trace in enumerate(traces):
        ...
        if (data.shape[1])>16:
            data=pd.read_csv(trace,sep='\t',usecols=[0,1,2,4,10,11,12,13,14,15,16,17],names=["chr", "start", "end",'weight',"hits1",'W1','array','c1','c2','c3','ChIPTF','gene'])
        else:
    ...
    #             table.append(column)
                ...
                np.transpose(pd.DataFrame((column))).to_csv(outdir+'sthlm_PRE_overall_TF5f.txt',mode='a')#,header=['cell','tf','mauroc','wauroc','meauroc','maupr','waupr','meaupr','size','max_hit','mo_length'])
            except ValueError:
                pass
                print('no match for '+Col1+"_"+Col2)
                np.transpose(pd.DataFrame((column))).to_csv(outidr+'sthlm_PRE_overall_TF5f.txt',mode='a')#,header=['cell','tf','mauroc','wauroc','meauroc','maupr','waupr','meaupr','size','max_hit','mo_length'])
