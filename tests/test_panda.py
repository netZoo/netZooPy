...
def test_panda():
    ...
    # Lazy test
    assert (np.round(panda_obj.panda_indegree.iloc[0].loc['force'], 5).as_type('float') == 1.13971)
    assert (np.round(panda_obj.panda_outdegree.iloc[0].loc['force'], 5).as_type('float') == 1030.06840)
